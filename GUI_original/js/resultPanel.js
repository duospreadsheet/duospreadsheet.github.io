var ResultPanel = {
	...
	updateQuestion: function() {
		...
		if (oneGroupOnTopShelf && oneGroupOnBottomShelf && !userSpecifiesAttributes) {
			...
			self.displayQuestion(questionHTML);
		}
			...
			let topShelfGroup = topShelfGroups[0];
			let questionHTML = TextGenerator.generateCompareOneAndManyQuestionHTML(topShelfGroup);
			self.displayQuestion(questionHTML);
		}
		else if (oneGroupOnTopShelf && manyGroupsOnBottomShelf && userSpecifiesAttributes) {
			let topShelfGroup = topShelfGroups[0];
			let attributeList = specifiedAttributes;
			let questionHTML = TextGenerator.generateCompareAttrOfOneAndManyQuestionHTML(topShelfGroup, attributeList);
			self.displayQuestion(questionHTML);
		}
		else {
			self.removeQuestion();
	...
	displayQuestion: function(questionHTML) {
	...
	updateAnswer: function() {
		...
		if (oneGroupOnTopShelf && oneGroupOnBottomShelf && !userSpecifiesAttributes) {
			...
			OneToManyAnswer.installMouseenterGroupNameBehaviour();
