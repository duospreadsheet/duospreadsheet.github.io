const FilterMenu = {
	init: function() {
		...
		function installClickHandleText() {
			...
			function updateSliderOnClickOutside(event) {
				...
				let currentValue = Math.round($(sliderRangeEditorSelector).val() * 100) / 100;
			...
			if (isCurrentAttrNumerical || !isCurrentAttrNumerical && isAttrValueSelected) {
				...
				if (!duplicatedFilter) {
					...
					setTimeout(function() {
						...
						FilterBar.installClickRemoveButtonBehaviour();
						Table.hideLoader();
					}, 10);
				}
	...
	installClickOutsideBehaviour: function() {
		...
		let filterMenuSelector = '#filter-menu, #filter-range-editor'; // filter range editor is part of filter menu
