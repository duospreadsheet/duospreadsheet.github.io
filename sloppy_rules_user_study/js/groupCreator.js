const GroupCreator = {
	...
	pushRulesToShelf: function(ruleObject, inputObject) {
		...
		let isAddingNewGroup = !isEditingGroup && !isViewingGroup; // only three possible cases
		...
		if (!isInputBoxEmpty) {
			...
			setTimeout(function() {
				let isAnswerCorrect = Shelf.checkIfMatchAPreset();

				Shelf.update();
				ResultPanel.update(correctAnswer = isAnswerCorrect);
