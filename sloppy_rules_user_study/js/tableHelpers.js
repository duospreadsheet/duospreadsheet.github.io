	...
	createHeaderHTML: function() {
		...
		for (let i = 0; i < numberOfColumns; i++) {
			...
			if (isCurrentAttrNumerical) {
				headerHTML += '<span class="column-index resizable rule-element" column-index="' + i + '" attribute-name="' + currentColumnName + '" lower-value="" upper-value="">' +
			...
			if (!isCurrentAttrNumerical) {
				headerHTML += '<span class="column-index resizable rule-element" column-index="' + i + '" attribute-name="' + currentColumnName + '" category="">' +
									'<span class="label">' + currentColumnIndex + ' (' + currentColumnName + ')' + '</span>' +
									'<span class="fas fa-caret-square-down"></span>' +
							  '</span>';
			}
	...
	createRowHTML: function(rowData) { // rowData: { index, data } or rowData: { index, data, added, removed }
		...
		for (let i = 0; i < numberOfRows; i++) {
			...
			for (let j = 0; j < numberOfColumns; j++) {
				...
				if (isCurrentAttrNumerical)
					currentRowHTML += '<span class="cell rule-element" attribute-name="' + currentAttributeName + '" lower-value="' + currentAttributeValue + '" upper-value="' + currentAttributeValue + '">' + currentAttributeValue + '</span>';
				if (!isCurrentAttrNumerical)
					currentRowHTML += '<span class="cell rule-element" attribute-name="' + currentAttributeName + '" category="' + currentAttributeValue + '">' + currentAttributeValue + '</span>';
			}
