const Shelf = {
	...
	filterRedundantRules_top: function(allRules) {
		const filteredRules = [];
		...
		for (let i = 0; i < allRules.length; i++) {
			...
			if (!isCurrentRulePartialSpecification)
				filteredRules.push(currentRule);
		}
	...
	addRulesFromInputBox: function() {
		...
		if (currentShelfType == 'top') { // replace
			let baseRules = RuleInputBoxTab.generateRules('base');
			let inclusionRules = RuleInputBoxTab.generateRules('inclusion');
			let exclusionRules = RuleInputBoxTab.generateRules('exclusion');
			let filteredBaseRules = self.filterRedundantRules_top(baseRules);
			let filteredInclusionRules = self.filterRedundantRules_top(inclusionRules);
			let filteredExclusionRules = self.filterRedundantRules_top(exclusionRules);

			let currentGroupName = self.generateGroupName(filteredBaseRules, filteredInclusionRules, filteredExclusionRules);
			let groupObject = {
				name: currentGroupName,
				ruleBasedName: currentGroupName,
				baseRules: filteredBaseRules,
				inclusionRules: filteredInclusionRules,
		...
		if (currentShelfType == 'bottom') {
			...
			let inclusionRules = RuleInputBoxTab.generateRules('inclusion');
			let exclusionRules = RuleInputBoxTab.generateRules('exclusion');
			let isPartialSpecification = baseRules.length == 1 && 
										 baseRules[0].indexOf('=??') != -1 && 
			...
			if (!isPartialSpecification) {
				...
				if (filteredBaseRules.length == 0 && filteredInclusionRules.length == 0 && filteredExclusionRules.length == 0)
					return;
			...
			if (isPartialSpecification) {
				let filteredRuleArray = self.filterRedundantRules(baseRules);
