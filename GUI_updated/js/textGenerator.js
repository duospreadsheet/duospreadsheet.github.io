const TextGenerator = {
	...
	generateCompareOneAndManyQuestionHTML: function(topShelfGroup, bottomShelfGroups) {
		const self = this;
		let topGroupNameHTML = '<div class="top-group-name group-or-attribute">' + self.generateGroupNameHTML(topShelfGroup) + '</div>';
		let bottomGroupNameHTML = '';

		for (let i = 0; i < bottomShelfGroups.length; i++)
			bottomGroupNameHTML += '<div class="bottom-group-name group-or-attribute">' + self.generateGroupNameHTML(bottomShelfGroups[i]) + '</div>';
	...
	generateAttributeListString: function(attributeList) {
		...
		for (let i = 0; i < attributeList.length; i++) {
			...
			if (!isLastName && isSecondLastName)
				attributeListString += currentAttributeName + '<br/><span class="operator-between-rule">AND</span>';
			if (isLastName)
				attributeListString += currentAttributeName;
		}
	...
	generateGroupNameHTML: function(shelfGroup, capitalizeFirstLetter = false) {
		...
		if (needSpecialName) {
			...
			for (let i = 0; i < inclusionRules.length; i++) {
				...
				specialName += '<br/><span class="operator-between-rule">OR</span>' + currentRuleHTML;
			}
			for (let i = 0; i < exclusionRules.length; i++) {
				let currentRule = exclusionRules[i];
				let currentRuleHTML = self.generateRuleHTML(currentRule);

				specialName += '<br/><span class="operator-between-rule">NOT</span>' + currentRuleHTML;
			}
