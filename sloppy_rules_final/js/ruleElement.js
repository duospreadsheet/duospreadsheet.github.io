		...
		function clickRuleElement(event) {
			...
			if (!hasClickedFilterRemoveButton && showNumericalMenu && !hasClickedColumnIndex) {
				...
				if (lowerValueString !== '' && upperValueString !== '') {
					...
					[ selection, range ] = GroupCreatorInputBox.Rule.changeSelected(shelfType, attributeName, attributeValueObject);
					setTimeout(function () { selection.addRange(range); $('#group-creator .input-box').focus(); }, 1); // hack to set focus
				}
			}
			...
			if (!hasClickedFilterRemoveButton && showAttributeMenu) {
				...
				[ selection, range ] = GroupCreatorInputBox.Rule.changeSelected(shelfType, attributeName);
				setTimeout(function () { selection.addRange(range); $('#group-creator .input-box').focus(); }, 1); // hack to set focus
			}
		}
	},
