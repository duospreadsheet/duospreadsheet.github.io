const GroupEditor = {
	...
	removeRuleFromGroupObject: function(shelfType, groupObject, rule) {
		let ruleType = shelfType + 'Rules';
		let tabType = null;
		let indexOfRuleInRuleArray = groupObject[ruleType].indexOf(rule);
		let indexOfRuleInRuleObject = null;
		let countNonePartialRule = -1;
		let splittedInput = null;

		// find tab type
		if (shelfType == 'base') tabType = 'and';
		if (shelfType == 'inclusion') tabType = 'add';
		if (shelfType == 'exclusion') tabType = 'remove';

		// find indexOfRuleInRuleObject
		for (let i = 0; i < groupObject.originalRuleObject[tabType].length; i++) {
			let currentRule = groupObject.originalRuleObject[tabType][i];
			let isCurrentRulePartialSpecification = (currentRule.indexOf('=??') != -1);

			if (!isCurrentRulePartialSpecification)
				countNonePartialRule++;
			if (countNonePartialRule == indexOfRuleInRuleArray) {
				indexOfRuleInRuleObject = i;
				break;
			}
		}

		// remove from rule array, ruleObject
		groupObject[ruleType].splice(indexOfRuleInRuleArray, 1);
		groupObject.originalRuleObject[tabType].splice(indexOfRuleInRuleObject, 1);

		// remove from input object
		splittedInput = groupObject.originalInputObject[tabType].split(';');
		splittedInput.splice(indexOfRuleInRuleObject, 1);
		groupObject.originalInputObject[tabType] = splittedInput.join(';');
	},
	installClickShelfBehaviour: function(event) {
		...
		function clickShelf(event) {
			...
			if (clickedRemoveButton) {
				...
				self.removeRuleFromGroupObject(shelfType, currentGroupObject, currentRule);
				...
				if (shelfType == 'base' && !noBaseRulesAfterRemoval) {
					...
					GroupViewer.lastOperation.operation = 'removing the base rule';
					Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'addFromExcluded' });
				}
				if (shelfType == 'base' && noBaseRulesAfterRemoval) { // remove the only base rule
					GroupViewer.lastOperation.addToViewer = 'excluded';
					GroupViewer.lastOperation.operation = 'removing the base rule';
					Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'removeFromIncluded' });
				}
				if (shelfType == 'inclusion') {
					GroupViewer.lastOperation.addToViewer = 'excluded';
					GroupViewer.lastOperation.operation = 'removing the inclusion rule';
					Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'removeFromIncluded' });
				}
				if (shelfType == 'exclusion') {
					GroupViewer.lastOperation.addToViewer = 'included';
					GroupViewer.lastOperation.operation = 'removing the exclusion rule';
					Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'addFromExcluded' });
				}
			...
			if (!clickedRule && !clickedResizeHandle) {
				...
				GroupCreator.show(shelfType, shelfTop + shelfHeight + 9, shelfLeft - 2, originalInputObject);
				$('#tooltip').removeClass('show');
			}
		}
	},
	...
	installClearButtonBehaviour: function() {
		...
		function clickClearButton(event) {
			...
			if (shelfType == 'base') {
				GroupViewer.lastOperation.addToViewer = 'excluded';
				GroupViewer.lastOperation.operation = 'removing the base rule(s)';
				Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'removeFromIncluded' });
			}
			if (shelfType == 'inclusion') {
				GroupViewer.lastOperation.addToViewer = 'excluded';
				GroupViewer.lastOperation.operation = 'removing the inclusion rule(s)';
				Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'removeFromIncluded' });
			}
			if (shelfType == 'exclusion') {
				GroupViewer.lastOperation.addToViewer = 'included';
				GroupViewer.lastOperation.operation = 'removing the exclusion rule(s)';
				Shelf.changedGroupObjects.push({ groupObject: currentGroupObject, updateMode: 'addFromExcluded' });
			}
	...
	installCloseButtonBehaviour: function() {
		...
		function clickCloseButton() {
			...
			let shortNameLength = isSelectedTagOnTopShelf ? 19 : 16;
	...
	installRestoreButtonBehaviour: function() {
		...
		function clickRestoreButton() {
			...
			self.currentGroupObject.originalInputObject = self.copiedCurrentGroupObject.originalInputObject;
			self.currentGroupObject.originalRuleObject = self.copiedCurrentGroupObject.originalRuleObject;
			...
			GroupEditor.update();
			ResultPanel.showLoader(); // result panel must update
			GroupViewer.lastOperation.addToViewer = null;
			GroupViewer.lastOperation.numberOfRecordsAdded = null;
			GroupViewer.lastOperation.operation = null;
