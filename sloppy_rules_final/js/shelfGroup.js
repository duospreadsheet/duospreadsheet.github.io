const ShelfGroup = {
	...
	installClickBehaviour: function() {
		...
		let ruleElementSelector = RuleElement.selector + ', #table-scrollable-area'; // scrollable area added to handle scroll bug: scrolling when input box open

		$(topGroupShelfSelector).click(clickShelf);
		$(bottomGroupShelfSelector).click(clickShelf);
		$(attributeShelfSelector).click(clickShelf);
		Body.registerNotClickEvent(shelfAndGroupCreatorSelector + ',' + groupEditorShelfSelector + ',' + ruleElementSelector + ',' + groupEditorButtonSelector, notClickShelf);
		...
		function clickShelf(event) {
			...
			if (clickedRemoveButton) {
				...
				let groupCreatorShelfType = $('#group-creator').attr('shelf-type');

				if (isClickedTagSelected) {
					if (groupCreatorShelfType == 'base' ||
						groupCreatorShelfType == 'inclusion' ||
						groupCreatorShelfType == 'exclusion') // base, inclusion and exclusion group creators are part of group editor
						GroupCreator.hide(); 
