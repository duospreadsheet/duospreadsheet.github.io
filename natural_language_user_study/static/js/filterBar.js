const FilterBar = {
	...
	installClickCloseButtonBehaviour: function() {
		const self = this;

		$('#table .filter-bar .content .filter .fa-times').on('click', clickCloseButton);

		function clickCloseButton() {
			let currentFilter = d3.select(this).datum();
			let currentFilterIndex = self.filters.indexOf(currentFilter);
			...
			setTimeout(function() {
				self.updateTable();
				self.updateCount();
				self.installClickCloseButtonBehaviour();
				Table.hideLoader();
			}, 10);
		}
	}
