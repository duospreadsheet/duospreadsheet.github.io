var ResultPanel = {
	...
	resize: function() {
		let questionInputBoxHeight = $('#sidebar .question-input-box').height() + 15 + 16; // 15 is padding
		let newResultPanelHeight = 'calc(100% - ' + questionInputBoxHeight + 'px)';
	...
	updateQuestion: function(correctAnswer = false) {
		...
		if (oneGroupOnTopShelf && oneGroupOnBottomShelf && !userSpecifiesAttributes) {
			...
			let topShelfGroup = topShelfGroups[0];
			let firstBottomShelfGroup = bottomShelfGroups[0];
			let secondBottomShelfGroup = bottomShelfGroups[1];
			let questionHTML = TextGenerator.generateCompareOneAndManyQuestionHTML(topShelfGroup, firstBottomShelfGroup, secondBottomShelfGroup);
			self.displayQuestion(questionHTML, correctAnswer);
		}
		else if (oneGroupOnTopShelf && manyGroupsOnBottomShelf && userSpecifiesAttributes) {
			let topShelfGroup = topShelfGroups[0];
			...
			let secondBottomShelfGroup = bottomShelfGroups[1];
			let attributeList = specifiedAttributes;
			let questionHTML = TextGenerator.generateCompareAttrOfOneAndManyQuestionHTML(topShelfGroup, firstBottomShelfGroup, secondBottomShelfGroup, attributeList);
			self.displayQuestion(questionHTML, correctAnswer);
		}
		else {
			self.removeQuestion();
	...
	displayCannotUnderstand: function() {
		$('#sidebar .result-panel .answer.header')
			.html('Sorry, couldn\'t understand your question.')
			.css('display', 'none')
			.fadeIn();
	},
